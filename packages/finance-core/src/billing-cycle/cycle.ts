...
/** Derives account closing/due days from authoritative statement dates (e.g. PDF/OFX import). */
export function billingDaysFromStatementDates(
  ...
  const closing =
    closingDate instanceof Date
      ...
      : dayjs.utc(closingDate).date()
  const due =
    dueDate instanceof Date ? dueDate.getUTCDate() : dayjs.utc(dueDate).date()
