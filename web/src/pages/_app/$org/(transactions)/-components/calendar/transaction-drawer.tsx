        ...
        <div className="flex flex-col gap-4 p-4 overflow-y-auto overflow-x-hidden">
          <Card>
            ...
            <CardContent className="grid gap-2 text-sm">
              <p><span className="font-medium">Título:</span> {transaction.title}</p>
              <p><span className="font-medium">Valor:</span> {currency}</p>
              <p>
                <span className="font-medium">Vencimento:</span>{' '}
                {format(new Date(transaction.dueDate), "d 'de' MMMM yyyy", { locale: ptBR })}
              </p>
              <p><span className="font-medium">Responsável:</span> {transaction.payTo}</p>
              <div className="flex flex-wrap gap-1">
                {transaction.tags.map(tag => (
                  <Badge key={tag.name} className="bg-muted text-foreground" style={{ backgroundColor: tag.color }}>
                    {tag.name}
                  </Badge>
                ))}
              </div>
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-2">
              <div className="h-32 overflow-y-auto border rounded-md p-2 text-sm flex flex-col gap-1">
                {messages.length ? (
                  messages.map((m, i) => (
                    <p key={i} className="bg-muted rounded px-2 py-1 w-fit">
                      {m}
                    </p>
                  ))
                ) : (
